...
var getWeather = function(city) {
    ...
    var coordinateApiURL = "http://api.openweathermap.org/geo/1.0/direct?q=" + city + "&appid=61601e19c5fa6e594bfdc849af6afd53";
    fetch(coordinateApiURL).then(function(response1) {
        response1.json().then(function(locationData) {
            console.log(locationData);
            var lat = locationData[0].lat;
            var lon = locationData[0].lon;
            var weatherApiURL = "https://api.openweathermap.org/data/2.5/onecall?lat=" + lat + "&lon=" + lon + "&units=imperial&appid=61601e19c5fa6e594bfdc849af6afd53";
...
var displayWeatherData = function(city, weatherData, persist) {
    ...
    var today = new Date();
    currentWeatherContainerEl.querySelector("#date").textContent = formatDate(today);
    currentWeatherContainerEl.querySelector("#temperature").textContent = weatherData.current.temp;
    currentWeatherContainerEl.querySelector("#wind").textContent = weatherData.current.wind_speed;
    currentWeatherContainerEl.querySelector("#humidity").textContent = weatherData.current.humidity;
    currentWeatherContainerEl.querySelector("#icon").src = "http://openweathermap.org/img/w/" + weatherData.current.weather[0].icon + ".png"
    ...
    for (var i = 1; i <= 5; i++) {
        ...
        iconEl.setAttribute("src", "http://openweathermap.org/img/w/" + weatherData.daily[i].weather[0].icon + ".png");
